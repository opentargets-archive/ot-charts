...
const CHAR_WIDTH = 11; // TODO: base on font-size
...
class Gecko extends React.Component {
  ...
  _geneSlots(genes, scale) {
    ...
    const endOfGene = gene => {
      ...
      const labelLength = (gene.symbol.length + 1) * CHAR_WIDTH;
      return transcriptLength > labelLength
        ? gene.end
        : scale.invert(scale(gene.start) + labelLength);
    };
    sortedGenes.forEach(gene => {
      const suitableSlots = slots.filter(
        slot => scale(gene.start) > scale(slot.end)
      );
  ...
  _trackConfig(geneSlotCount, tagVariantBlockSlotCount, studySlotCount) {
