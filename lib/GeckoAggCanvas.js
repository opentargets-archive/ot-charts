...
class Gecko extends React.Component {
  ...
  _renderGeneIndexVariantStudies(
    ...
    data.forEach(d => {
      ...
      // gene - indexVariant
      {
        const gIvTopX = scaleX(d.geneTss);
        const gIvTopY = gTvTrack.top;
        const gIvBottomX = scaleX(d.indexVariantPosition);
        const gIvBottomY = tvIvTrack.bottom;
        const gIvControlY = (gIvBottomY + gIvTopY) / 2;
        context.strokeStyle = connectorColor(d, noSelectedEntities);
        context.beginPath();
        context.moveTo(gIvTopX, gIvTopY);
        context.bezierCurveTo(
          gIvTopX,
          gIvControlY,
          gIvBottomX,
  ...
  _trackConfig(geneSlotCount, tagVariantBlockSlotCount, studyTrackHeight) {
    ...
    const tagVariantBlockTrackHeight = 0;
    ...
    const tagVariantIndexVariants = {
      ...
      height: 0, //CONNECTOR_TRACK_HEIGHT,
    };
    const indexVariants = {
