...
class Gecko extends React.Component {
  ...
  _geneSlots(genes, scale) {
    ...
    const endOfGene = gene => {
      ...
      const labelLength = gene.symbol.length * CHAR_WIDTH;
      return transcriptLength > labelLength
        ? scale.invert(scale(gene.end) + GENE_BACKDROP_PADDING)
        : scale.invert(scale(gene.start) + labelLength + GENE_BACKDROP_PADDING);
    };
