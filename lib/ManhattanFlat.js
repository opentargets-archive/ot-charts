...
import { chromosomesWithCumulativeLengths, SIGNIFICANCE } from './utils';
...
class ManhattanFlat extends React.Component {
  constructor(props) {
    super(props);
    this.svgRef = React.createRef();
  }
  render() {
    const { measureRef, data } = this.props;
                  ...
                  return (
                    <React.Fragment key={`chr${d.name}`}>
                      <rect
                        ...
                        y={0}
                        width={
                          (d.proportionalEnd - d.proportionalStart) * width
                        }
                        height={height}
                        fill={
                          i % 2 === 0
                      ...
                      {width > chromosomeLabelWidthThreshold ? (
                        <text
                          x={
                            ((d.proportionalStart + d.proportionalEnd) *
                              width) /
                            2
                          }
                          y={height / 2}
                ...
                {data.map(d => {
                  ...
                  return (
                    <line
                      ...
                      y2={height}
                      stroke={
                        -Math.log10(d.pval) > SIGNIFICANCE
                          ? theme.secondary
                          : theme.line.color
                      }
                      strokeWidth={2}
...
ManhattanFlat = withContentRect('bounds')(ManhattanFlat);
