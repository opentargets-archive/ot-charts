...
class Manhattan extends React.Component {
  ...
  _render() {
    ...
    const { data, focusChromosome, handleClick, handleMouseover } = this.props;
    const { width, height } = this._dimensions();
    if (
      !data ||
      !data.associations ||
    ...
    const { scaleMinusLogPValue, scaleChromosomes, chartIndex } = this.state;
  ...
  _renderAxisLabelChromosome(svg, chromosome, chartIndex, translation) {
    const { handleChromosomeClick, focusChromosome } = this.props;
    // render in own group
    let g = svg.select(
    ...
    if (g.empty()) {
      ...
      g.append('rect')
        ...
        .attr('fill-opacity', 0)
        .on('click', () =>
          handleChromosomeClick(focusChromosome ? null : chromosome.name)
        )
